(() => {const CACHE_NAME = 'mylhfBlogCache';
...
const matchCacheRule = (url) => {
      ...
      url.pathname.match(/\.(woff2|woff|ttf|cur|webp|png|jpg|jpeg|css|js)$/);
    };
...
const getFastestRequests = (request) => {
    ...
    if (srcUrl.startsWith('https://cdn.jsdelivr.net')) {
      ...
      return [
      `https://gcore.jsdelivr.net` + url.pathname,
      `https://cdn.jsdmirror.com` + url.pathname];

    }
    if (srcUrl.startsWith('https://gcore.jsdelivr.net')) {
      const url = new URL(srcUrl);
      return [
      `https://gcore.jsdelivr.net` + url.pathname,
      `https://cdn.jsdmirror.com` + url.pathname];
    ...
    if (srcUrl.startsWith('https://cdn.cbd.int')) {
      ...
      return [
      srcUrl,
      `https://cdn.jsdmirror.com/npm` + url.pathname];
...
const isBlockRequest = (request) => {
    const srcUrl = request.url;
    return srcUrl.startsWith('https://cdn.bootcss.com');
  };
